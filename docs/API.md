...
Admin routes require `X-Admin-Key` and expose dashboard, jobs, staged discovery (paged checkpoints or exact source identifiers), rights approval/rejection, cover/review/contact queues, quality, storage, SEO status, takedowns, and monetization readiness. Novel operations include fail-closed publish/unpublish, ad controls, chapter reprocessing, guarded cover regeneration, and non-destructive duplicate merging into a canonical work. Never place the admin key in client source or Git. The local admin page stores it in session storage only.
